...
import { isCompactUiViewport, truncateGuildNote as truncateGuildNoteText, appendGuildNote } from '../systems/hudSystem';
import { getSplashOverlayLayout as getSplashOverlayLayoutConfig, getLevelUpOverlayLayout as getLevelUpOverlayLayoutConfig, getGameOverOverlayLayout as getGameOverOverlayLayoutConfig, getOverlayContentOffset as getOverlayContentOffsetValue, isCompactOverlayLayout as isCompactOverlayLayoutForViewport } from '../systems/overlaySystem';
import { appendPlayerMovementTrace, resolveAutoTargetIso } from '../systems/playerSystem';
import { getGeneratedTerrainApronOutsideDistance as getGeneratedTerrainApronOutsideDistanceValue, isGeneratedTerrainApronGateSightline as isGeneratedTerrainApronGateSightlineValue, shouldRenderGeneratedCellCliff as shouldRenderGeneratedCellCliffForBounds, getGeneratedWorldCliffFrame as getGeneratedWorldCliffFrameForBounds, getGeneratedWorldCliffOffset as getGeneratedWorldCliffOffsetForBounds, isGeneratedBoardEdgeCell as isGeneratedBoardEdgeCellForBounds, getGeneratedWorldBounds as getGeneratedWorldBoundsForLevel } from '../systems/worldRenderSystem';
import {
...
export class FairyGuildScene extends Phaser.Scene {
  declare player: any;
  declare playerStats: any;
  declare state: any;
  declare heroClass: HeroClass;
  declare heroAnimPrefix: string;
  declare keys: Record<string, any>;
  declare enemies: any[];
  declare projectiles: any[];
  declare buildings: any[];
  declare playerOccluders: PlayerOccluder[];
  declare effects: any[];
  declare notes: string[];
  declare cardTiers: any;
  declare levelUpChoices: any[];
  declare lastSelectedCard: CardKey | null;
  declare lastOfferedCards: CardKey[];
  declare runStats: { enemiesDefeated: number };
  declare heartDropDefeatStreak: number;
  declare traps: any[];
  declare guardUntil: number;
  declare magicShield: any;
  declare roundEnemyQueue: Array<EnemyRoleKey | 'boss'>;
  declare levelUpChoiceCards: any[];
  declare lastPointerIso: GridPoint;
  declare lastPlayerSafeIso: GridPoint | null;
  declare lastPlayerMovementIntent: any;
  declare lastRejectedMovementReason: string | null;
  declare lastPlayerMovementResult: any;
  declare previousPlayerSlideScreen: any;
  declare playerMovementTrace: any[];
  declare debugEnemiesFrozen: boolean;
  declare levelSpawnsPending: number;
  declare levelEnemiesRemaining: number;
  declare levelRequiredDefeats: number;
  declare levelDefeatsThisRound: number;
  declare levelSpawnFailures: number;
  declare levelClearQueued: boolean;
  declare levelSpawnedCount: number;
  declare levelTimers: any[];
  declare generatedValidSpawnPoints: GridPoint[];
  declare lastRepairAt: number;
  declare touchControls: any;
  declare touchControlsEnabled: boolean;
  declare touchDetection: any;
  declare lastTouchControlsVisibility: boolean | null;
  declare controlsHint: any;
  declare debugOverlay: any;
  declare debugOverlayVisible: boolean;
  declare debugOverlayExpanded: boolean;
  declare generatedLevel: any;
  declare generatedLevelValidation: any;
  declare generatedLevelActive: boolean;
  declare generatedLevelConfigId: string;
  declare generatedLevelConfigLabel: string;
  declare generatedLevelSelectionWarnings: string[];
  declare levelDebugLayer: any;
  declare levelDebugGraphics: any;
  declare levelDebugLabels: any[];
  declare levelDebugVisible: boolean;
  declare levelDebugLastRenderAt: number;
  declare generatedTerrainMask: any;
  declare generatedTerrainMaskGraphics: any;
  declare generatedApronDebugCells: any[];
  declare generatedApronDebugStats: any;
  declare lightingLayer: any;
  declare timeOfDayOverlay: any;
  declare timeOfDayMist: any;
  declare lampGlowGraphics: any;
  declare timeOfDayOverride: any;
  declare sceneVariant: SceneVariantConfig | null;
  declare sceneVariantOverrideKey: string | null;
  declare resumeRunState: RunResumeStateSnapshot | null;
  declare resumeSkipSplash: boolean;
  declare forceFreshStart: boolean;
  declare autoSelectHero: boolean;
  declare preselectHero: boolean;
  declare parallaxSprites: any[];
  declare worldCamera: Phaser.Cameras.Scene2D.Camera | null;
  declare uiCamera: Phaser.Cameras.Scene2D.Camera | null;
  declare cameraParallaxOrigin: { x: number; y: number } | null;
  declare ambientParticles: any[];
  declare ambientParticlePreset: any;
  declare nextAmbientParticleAt: number;
  declare audio: any;
  declare backgroundLayer: any;
  declare shadowLayer: any;
  declare terrainBaseLayer: any;
  declare edgeLayer: any;
  declare waterLayer: any;
  declare decorLayer: any;
  declare buildingLayer: any;
  declare characterLayer: any;
  declare effectsLayer: any;
  declare hudLayer: any;
  declare touchLayer: any;
  declare worldLayer: any;
  declare entityLayer: any;
  declare fxLayer: any;
  declare uiLayer: any;
  declare tileGraphics: any;
  declare pendingHeroClass: HeroClass | null;
  declare splashOverlay: any;
  declare splashHeroChoiceText: any;
  declare splashStartButton: any;
  declare splashStartText: any;
  declare splashHeroCards: Partial<Record<HeroClass, any>>;
  declare countdownOverlay: any;
  declare countdownLevelText: any;
  declare countdownNumberText: any;
  declare levelUpOverlay: any;
  declare levelUpTitleFit: any;
  declare levelUpTitleText: any;
  declare levelUpRewardText: any;
  declare levelUpHelperText: any;
  declare gameOverOverlay: any;
  declare gameOverReasonText: any;
  declare gameOverStatsText: any;
  declare hud: any;
  ...
  getGeneratedTerrainApronOutsideDistance(x, y) {
    return getGeneratedTerrainApronOutsideDistanceValue(x, y, this.generatedLevel);
  }

  isGeneratedTerrainApronGateSightline(x, y) {
    return isGeneratedTerrainApronGateSightlineValue(x, y, this.generatedLevel);
  }
  ...
  shouldRenderGeneratedCellCliff(grid) {
    return shouldRenderGeneratedCellCliffForBounds(grid, this.generatedLevel?.playableBounds ?? null);
  }
  ...
  getGeneratedWorldBounds(tileW, tileH) {
    return getGeneratedWorldBoundsForLevel(
      this.generatedLevel,
      tileW,
      tileH,
      (x, y) => this.isoToScreen(x, y),
    );
  }
  ...
  getGeneratedWorldCliffFrame(grid) {
    return getGeneratedWorldCliffFrameForBounds(grid, this.generatedLevel?.playableBounds ?? null);
  }

  getGeneratedWorldCliffOffset(grid, tileW, tileH) {
    return getGeneratedWorldCliffOffsetForBounds(
      grid,
      tileW,
      tileH,
      this.generatedLevel?.playableBounds ?? null,
    );
  }

  isGeneratedBoardEdgeCell(grid) {
    return isGeneratedBoardEdgeCellForBounds(grid, this.generatedLevel?.playableBounds ?? null);
  }
  ...
  getScreenMovementVector() {
    return getScreenMovementVectorFromInput({
      ...
      joystickVisible: this.touchControls?.container.visible,
      joystickVector: this.touchControls?.joystickVector,
    });
  }

  getAutoTargetIso(maxRange: number) {
    return resolveAutoTargetIso(
      this.player.iso,
      this.player.facing,
      this.enemies,
      maxRange,
      (point, padding) => this.clampIso(point, padding),
    );
  }
  ...
  recordPlayerMovementTrace(result) {
    ...
    this.playerMovementTrace = appendPlayerMovementTrace(
      this.playerMovementTrace,
      this.player.iso,
      result,
      PLAYER_MOVEMENT_TRACE_LIMIT,
    );
  }
  ...
  isCompactOverlayLayout() {
    ...
    return isCompactOverlayLayoutForViewport(this.touchControlsEnabled, {
      width: viewportWidth,
  ...
  getOverlayContentOffset(panelHeight) {
    return getOverlayContentOffsetValue(panelHeight);
  }

  getSplashOverlayLayout() {
    return getSplashOverlayLayoutConfig(this.isCompactOverlayLayout());
  }

  getLevelUpOverlayLayout() {
    return getLevelUpOverlayLayoutConfig(this.isCompactOverlayLayout());
  }

  getGameOverOverlayLayout() {
    return getGameOverOverlayLayoutConfig(this.isCompactOverlayLayout());
  }
  ...
  getGeneratedRouteDebugSummary() {
    ...
    return formatGeneratedRouteDebugSummary(this.getGeneratedRouteScores(source));
  }

  getLiveEnemyTargetSummary() {
    return getLiveEnemyTargetSummaryText(this.enemies);
  }
  ...
  updateDebugOverlay() {
    ...
    const buildingSummary = getBuildingDebugSummary(this.buildings);
    const activeEnemies = countActiveEnemies(this.enemies);
    const metrics = this.getIsoMetrics();
  ...
  isCompactUi() {
    const viewportWidth = window.visualViewport?.width ?? window.innerWidth;
    return isCompactUiViewport(this.touchControlsEnabled, viewportWidth);
  }

  truncateGuildNote(note, maxLength) {
    return truncateGuildNoteText(note, maxLength);
  }
  ...
  addGuildNote(message) {
    this.notes = appendGuildNote(this.notes, message);
  }
}
