...
- The painted board remains the default map while the generator is introduced safely. Append `?generatedLevel=1` for the default generated map or `?generatedLevel=festival-village` for a second catalog level. Query overrides such as `?seed=my-seed`, `?density=0.6`, `?difficulty=2`, `?tileSize=64`, and `?timeOfDay=night` help preview designer variants. Generated maps honor `tileSize` by scaling isometric diamond spacing and generated object art while leaving the static painted board unchanged. Append `?debugLevel=1` or press `G` to show logical grid cells, blockers, protected building footprints, spawn points, attack cells, validation routes, chests, decorations, and live enemy paths. The `B` balance panel includes generated-level route scores, tile metrics, and live target counts so designers can inspect monster target selection.
- Generated levels include a deterministic decoration pass that adds nonblocking flowers, mushrooms, sparkles, and magical plants around the authored village without changing walkability. Append `?timeOfDay=morning`, `?timeOfDay=noon`, `?timeOfDay=afternoon`, or `?timeOfDay=night` to preview visual-only lighting and glow profiles; press `N` to cycle those profiles at runtime.
