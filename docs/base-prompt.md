...
- Generated-map world art is packed into deterministic fixed-cell Phaser atlases: `public/assets/world_tiles_atlas.png/json`, `buildings_atlas.png/json`, `ui_atlas.png/json`, `touch_controls_atlas.png/json`, `hud_ui_atlas.png/json`, `hud_bars_atlas.png/json`, and `effects_atlas.png/json`. The legacy `world-ui-sheet.png` is source art only; runtime generated maps should use atlas texture/frame keys so trees, terrain, buildings, UI icons, and effects are not cropped from inconsistent collage coordinates.
...
- The generated world atlas includes directional `island_edge_*`, `island_corner_*`, and `island_shadow_01` frames so procedural boards have an intentional isometric side wall and soft shadow instead of ending abruptly against the blue/green page background.
