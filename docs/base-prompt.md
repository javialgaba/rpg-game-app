...
- The painted board remains the default map while the generator is introduced safely. Append `?generatedLevel=1` to render from the token matrix using sliced `world-ui-sheet.png` frames where available, and `?debugLevel=1` to show logical grid cells, blockers, protected building footprints, spawn points, attack cells, and validation routes.
